...
public class Stone extends Entity{
	...
	public void stoneFall() {
		...
		if(colP == col && rowP == row +1) {
			...
			System.out.println("Spieler unter stein");
		}
		else {
		...
		if(isFalling && playerUnderStone) {
			...
			} else if (isFalling && !(gp.tileM.mapTileNum[col][row + 1] == gp.tileM.backBlackID)){
				gp.playSE("stoneFall");
				isFalling = false;
			}
		}
	...
	public void update() {
		screenXY();
		stoneFall();  //Auskommentieren falls es zu problemen führt
	}
